...
// rdataName builds rdata for a CNAME or NS record. label identifies errors,
// and field is the UDDI rdata key.
...
// quoteTXT wraps an unquoted TXT value so UDDI stores one character-string.
...
// unquoteTXT removes one pair of double quotes and unescapes `\"`. It leaves
// other values unchanged. This form matches the text stored by the Portal.
func unquoteTXT(s string) string {
	if len(s) < 2 || !strings.HasPrefix(s, `"`) || !strings.HasSuffix(s, `"`) || strings.HasSuffix(s, `\"`) {
		return s
	}
	inner := s[1 : len(s)-1]
	// An inner bare quote separates character-strings, so preserve the value.
